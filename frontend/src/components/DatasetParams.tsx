...
interface FeatureColumnsProps {
  ...
  onChange: (ignore_columns: number[]) => void;
  disabled?: boolean;
...
export function FeatureColumns({ dataset, ignoreColumns, onChange, disabled, clipboard, onCopy, onPaste }: FeatureColumnsProps) {
  ...
  const handleColumnToggle = (index: number) => {
    if (ignoreColumns.includes(index)) {
      onChange(ignoreColumns.filter(i => i !== index));
    } else {
      onChange([...ignoreColumns, index]);
    }
  };

  const allSelected = !ignoreColumns || ignoreColumns.length === 0;

  const handleSelectAll = () => {
    if (allSelected) {
      // Deselect all - add all column indices to ignore_columns
      onChange(columns.map((_, i) => i));
    } else {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-sm font-medium text-gray-700">Feature Columns</h3>
      ...
      <div className="flex flex-wrap gap-2">
        {columns.map((col, index) => (
          <Checkbox
            key={col}
            label={col}
            checked={!ignoreColumns?.includes(index)}
